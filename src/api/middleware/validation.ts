  ...
  try
  {
    ...
    for (const path of Object.keys(openApiDefinition.paths))
    {
      ...
      if (operation.requestBody && req.headers['content-type'])
      {
        ...
        const requestBodySchema = requestBody.content[req.headers['content-type'].split(';')[0]].schema;
        if (requestBodySchema['$ref'])
        {
          const requestBodyReference = requestBodySchema as Reference;
          const componentName = requestBodyReference.$ref.substring(requestBodyReference.$ref.lastIndexOf('/') + 1);
